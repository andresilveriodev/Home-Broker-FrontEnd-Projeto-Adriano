...
import { MUDAR_ORDEM_EXEC_MAIN_REDUCER } from "./MenuActionTypes";
import store from './StoreCreation'; 
...
export const atualizarShowAction = (show) => {
  store.dispatch({ type: ATUALIZAR_SHOW, payload: show });
};
...
const criarMostrarApp = (name, props, codigo_ativo) => {
  ...
  store.dispatch(atualizarDivKeyAction(`${name}${apps.length}`));
  show.push({
    book: false,
...
const mostrarApp = (name, index, props, codigo_ativo) => {
  ...
  let apps = [...props.apps];
  let show = [...props.show];
  store.dispatch(atualizarDivKeyAction(`${name}${index}`));
...
export const aumentarZindex = (zIndex) => {
  zIndex = zIndex + 1;
  store.dispatch({ type: AUMENTAR_ZINDEX, payload: zIndex });
};
